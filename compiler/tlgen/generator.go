...
const generatedHeader = "// Code generated by tlgen. DO NOT EDIT."
...
func writeGoFile(path string, content string) error {
	...
	return writeFileAtomic(path, formatted, 0o644)
}
...
func cleanupGeneratedFiles(outDir string, patterns []string) error {
	...
	for _, e := range entries {
		...
		for _, p := range patterns {
			...
			if err != nil {
				return fmt.Errorf("invalid cleanup pattern %q: %w", p, err)
			}
			if match {
				if err := os.Remove(filepath.Join(outDir, name)); err != nil {
					return fmt.Errorf("remove generated file %s: %w", name, err)
				}
				break
			}
		}
...
// InstallGeneratedFiles installs a fully generated tree without touching
// hand-written files in outDir. New files are written before stale generated
// files are removed, so generation failures leave the existing tree intact.
func InstallGeneratedFiles(outDir, generatedDir string) error {
	...
	if err := filepath.WalkDir(generatedDir, func(path string, entry os.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		if entry.IsDir() {
		...
		files = append(files, generatedFile{rel: rel, data: data, perm: info.Mode().Perm()})
		generated[rel] = struct{}{}
		return nil
	}); err != nil {
		return fmt.Errorf("read generated tree: %w", err)
	}

	sort.Slice(files, func(i, j int) bool { return files[i].rel < files[j].rel })
	for _, file := range files {
		target := filepath.Join(outDir, file.rel)
		if err := os.MkdirAll(filepath.Dir(target), 0o755); err != nil {
			return err
		}
		if err := writeFileAtomic(target, file.data, file.perm); err != nil {
			return fmt.Errorf("install generated file %s: %w", file.rel, err)
		}
	}

	if err := filepath.WalkDir(outDir, func(path string, entry os.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		if entry.IsDir() {
		...
		return fmt.Errorf("remove stale generated files: %w", err)
	}
	return nil
}
...
func e2ePrefixReadCode(code string) string {
	replacements := []struct{ from, to string }{
		...
		{"checkVectorConstructor(", "tg.CheckVectorConstructor("},
