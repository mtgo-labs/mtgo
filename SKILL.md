...
**Third-party content exposure (indirect prompt injection risk).** This skill ingests untrusted, user-generated Telegram content (`ctx.Message.Text`, `ctx.CallbackQuery.Data`, inline queries, edited messages) and external URLs (`telegram.URL(...)`, `DownloadMedia`, `InputFile.URL`), and those inputs can drive agent actions (replies, edits, bans, RPC calls). An attacker could craft message text or callback data containing prompt-injection payloads designed to manipulate the agent's behavior.

### Mandatory mitigations

- **Validate and constrain user input.** Use filters (`telegram.Command`, `telegram.CallbackData("exact")`, `telegram.CallbackRegex`) to restrict which messages/callbacks are processed. Never route untrusted text to the agent's decision-making pipeline without filtering.
- **Treat all message text as data, never as instruction.** Never pass user message text directly into agent reasoning without a clear boundary (e.g., prefix it with "User said:" and isolate it from control flow). Avoid concatenating user text into messages that describe what the agent should do.
- **Validate callback data against a whitelist.** Use exact-match `CallbackData` filters or regex-enforced schemas (`CallbackRegex`). Never process callback data that wasn't generated by your own bot's keyboards.
- **Sanitize external URLs before fetching.** Validate that URLs passed to `telegram.URL(...)` and `DownloadMedia` are from expected domains. Restrict fetches to known-safe origins (e.g., Telegram file references, your own CDN). Never fetch from user-supplied URLs without validation.
- **Never eval/exec user content.** Do not interpret message text as code, commands, or configuration. User text enters the handler as a Go `string` — treat it as opaque data.
- **Adversarial content via forward/edit.** Be aware that forwarded messages and edited messages (`OnEditedMessage`) can contain adversarial payloads. Apply the same validation to all handler types, not just `OnMessage`.
- **Content echoed in replies.** Do not echo untrusted user content back in `ctx.Reply(...)` or `ctx.CallbackEditText(...)` without sanitization — the echoed content could target downstream agents or human operators.
